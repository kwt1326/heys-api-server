...
class UserDetailController (
    ...
    @GetMapping("/me")
    fun getMyInfo(@Schema(hidden = true) @RequestHeader(HttpHeaders.AUTHORIZATION) bearer: String) : ResponseEntity<CommonApiResponse<UserDetailResponse>?>? {
        val userDetailResponse = userDetailService.getMyInfo(bearer)
        return ResponseEntity.ok(CommonApiResponse(data = userDetailResponse))
    }
    ...
    fun modifyMyInf(@Schema(hidden = true) @RequestHeader(HttpHeaders.AUTHORIZATION) bearer: String,
                    @RequestBody body: UserDetailRequest): ResponseEntity<CommonApiResponse<Any>?>? {
    ...
    fun getOtherUserInfo(@Schema(hidden = true) @RequestHeader(HttpHeaders.AUTHORIZATION) bearer: String,
                         @PathVariable(name = "userId") userId: Long) : ResponseEntity<CommonApiResponse<OtherUserDetailResponse>?>? {
        val otherUserDetailResponse = userDetailService.findOtherUserDetail(userId)
        return ResponseEntity.ok(CommonApiResponse(data = otherUserDetailResponse))
    }
}